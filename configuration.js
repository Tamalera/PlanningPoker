...
function checkIfViable() {
    ...
    for (var i = 1; i < flipCard.length; i++) {
        if (flipCard[i].children[0].checked) {
            counter++;
            document.getElementById("ausWahlAnzeigen").value = flipCard[i].children[0].value.toString();
        }
    }
    if (counter === 1) {
        document.getElementById("ausWahlBestaetigenButton").disabled = false;
        document.getElementById("ausWahlBestaetigenButton").textContent = "Gültige Wahl!";
        document.getElementById("ausWahlBestaetigenButton").className = "btn btn-success text-white";
    } else {
        document.getElementById("ausWahlBestaetigenButton").disabled = true;
        document.getElementById("ausWahlBestaetigenButton").textContent = "Ungültige Wahl!";
        document.getElementById("ausWahlBestaetigenButton").className = "btn btn-danger text-white";
    }
}
